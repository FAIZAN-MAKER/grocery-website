...
import {
  Search, ShoppingCart, UserIcon, X, LogOut, ChevronDown,
...
// ─── Dropdown Menu Item ───────────────────────────────────────────────────────
const MenuItem = ({
  children, onClick, className = "", href, icon,
}: {
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          {/* Panel */}
          <motion.aside
            className="fixed top-0 right-0 h-full w-[300px] bg-white z-50 flex flex-col shadow-2xl"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
              ...
              <div className="flex items-center gap-3">
                <div className="relative shrink-0">
                  {user.image ? (
                    ...
                    <div className="w-12 h-12 rounded-xl bg-green-500 flex items-center justify-center text-white font-bold text-base ring-2 ring-green-200">
                      {initials}
                    </div>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <span className="inline-block mt-1.5 text-[10px] font-semibold uppercase tracking-wider bg-green-500 text-white px-2 py-0.5 rounded-full">
                    Admin
                  </span>
                </div>
              </div>
              ...
              {adminLinks.map(({ href, icon: Icon, label, desc }, i) => (
                ...
                >
                  <Link href={href} onClick={onClose}>
                    <motion.div
                      className="flex items-center gap-3 px-3 py-3 rounded-2xl text-gray-700 hover:bg-green-50 hover:text-green-700 transition-colors group"
        ...
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 shadow-lg shadow-green-900/20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
            ...
            <Link href="/" className="flex items-center gap-2.5 shrink-0 group" aria-label="SnapCart Home">
              ...
              <span className="text-white font-bold text-xl tracking-tight hidden sm:block group-hover:tracking-wider transition-all duration-300">
                SnapCart
              </span>
            </Link>

            {/* Admin desktop buttons (lg+ only) */}
            {isAdmin && (
            ...
            {/* User/DeliveryBoy search bar */}
            {!isAdmin && (
              <motion.form
                className="hidden md:flex flex-1 max-w-lg items-center gap-3 bg-white/10 border border-white/20 rounded-2xl px-4 py-2.5 focus-within:bg-white/20 focus-within:border-white/40 transition-all duration-200"
              ...
              >
                ...
                <AnimatePresence>
                  {searchQuery && (
                    <motion.button type="button" onClick={() => setSearchQuery("")}
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {/* Mobile search — non-admin only */}
              {!isAdmin && (
                <motion.button
                  className="md:hidden w-10 h-10 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-xl text-white transition-colors"
              ...
              {!isAdmin && (
                <Link href="/cart">
                  <motion.div
                    className="relative flex items-center justify-center w-10 h-10 bg-white/10 hover:bg-white/20 rounded-xl text-white transition-colors"
              ...
              {/* Admin hamburger — small/medium screens only */}
              {isAdmin && (
                <motion.button
                  className="lg:hidden w-10 h-10 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-xl text-white transition-colors"
              ...
              <div className="relative" ref={dropdownRef}>
                <motion.button
                  className="flex items-center gap-2.5 bg-white/10 hover:bg-white/20 rounded-xl px-2.5 py-1.5 transition-colors"
                  ...
                  {dropdownOpen && (
                    ...
                    >
                      ...
                      <div className="p-2 space-y-0.5">
                        {/* My Orders — user & deliveryBoy only */}
                        {!isAdmin && (
                          <MenuItem
                            href="/orders"
          ...
          {!isAdmin && (
            <AnimatePresence>
              {searchOpen && (
                ...
                >
                  <form
                    className="flex items-center gap-3 bg-white/10 border border-white/20 rounded-2xl px-4 py-3"
                    onSubmit={(e) => e.preventDefault()}
                  >
